...
class Material(URDFType):
    ...
    def _from_xml(cls, node, path):
        ...
        if color is not None:
            color = np.fromstring(color.attrib['rgba'], sep=' ', dtype=float)
        kwargs['color'] = color
...
class Joint(URDFType):
    ...
    def axis(self, value):
        if value is None:
            value = np.array([1.0, 0.0, 0.0], dtype=float)
    ...
    def get_child_pose(self, cfg=None):
        ...
        if cfg is None:
            ...
            if cfg is None:
                cfg = np.zeros(2, dtype=float)
            else:
                cfg = np.asanyarray(cfg, dtype=float)
            if cfg.shape != (2,):
                raise ValueError(
...
class URDF(URDFType):
    ...
    def link_fk_batch(self, cfgs=None, link=None, links=None, use_names=False):
        ...
        for lnk in self._reverse_topo:
            if lnk not in link_set:
                continue
            poses = np.tile(np.eye(4, dtype=float), (n_cfgs, 1, 1))
            path = self._paths_to_base[lnk]
            for i in range(len(path) - 1):
                child = path[i]
                parent = path[i + 1]
    ...
    def animate(self, cfg_trajectory=None, loop_time=3.0, use_collision=False):
        ...
        # If it is specified, parse it and extract the trajectory length.
            ...
            ct = np.asanyarray(ct).astype(float)
            if ct.ndim == 1:
                ct = ct.reshape(-1, 1)
    ...
    def _process_cfgs(self, cfgs):
        ...
        if isinstance(cfgs, dict):
            ...
            if isinstance(cfgs[0], dict):
                ...
                cfgs = np.asanyarray(cfgs, dtype=float)
